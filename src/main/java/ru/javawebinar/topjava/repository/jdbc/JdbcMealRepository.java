...
public class JdbcMealRepository implements MealRepository {
    ...
    public List<Meal> getAll(int userId) {
        List<Meal> meals = jdbcTemplate.query("SELECT * FROM meals WHERE user_id=? ORDER BY date_time ",
                ROW_MAPPER, userId);
    ...
    public List<Meal> getBetweenHalfOpen(LocalDateTime startDateTime, LocalDateTime endDateTime, int userId) {
        List<Meal> meals = jdbcTemplate.query("SELECT * FROM meals WHERE user_id=? ",
                ROW_MAPPER, userId);
        meals = filterByPredicate(meals, meal -> Util.isBetweenHalfOpen(meal.getDateTime(), startDateTime, endDateTime));
        return meals;
    }
